...
from function import SpatialTransformer, upsample
            ...
            for j in range(num_shifts):
                for k in range(num_shifts):
                    prod = (
                        features1
                        * features2_padded[
                            :, :, i : (height + i), j : (width + j), k : (depth + k)
                        ]
                    )
...
class voxelencoder(nn.Module):
    ...
    def forward(self, x):
        x1 = self.conv0(x)  # 连接   （1倍）
        x2 = self.conv1(x1)  # 1 -> 1/2
        x3 = self.conv2(x2)  # 连接 （2倍）
        x4 = self.conv3(x3)  # 1/2 -> 1/4
        x5 = self.conv4(x4)  # 连接   (4倍)
        x6 = self.conv5(x5)  # 1/4 -> 1/8
        x7 = self.conv6(x6)  # 最低尺度特征 (8倍plans)

        return x1, x3, x5, x7


class offsetTransformer(nn.Module):
    def __init__(self, size, group, mode="bilinear"):
    ...
    def forward(self, src, flow, offset):
        C = src.shape[1]
        src_all = torch.split(src, C // self.group, dim=1)
        offset_all = torch.split(offset, 3, dim=1)

        srcs = []
        for i in range(self.group):
            offset_i = offset_all[i]
            src_i = src_all[i]
            shape = flow.shape[2:]
            new_locs = self.grid + offset_i + flow
            ...
            """
            src_i = F.grid_sample(src_i, new_locs, mode=self.mode, align_corners=True)
            srcs.append(src_i)
...
class group_flow(nn.Module):
    ...
    def __init__(self, in_plans, plans, group=4):
        ...
        # self.channelattention = ChannelAttention(plans, ratio=4)
        self.flowout = nn.Conv3d(plans, 3, kernel_size=3, padding=1, stride=1)
        # self.flowfusion = flowfusion(plans, ratio=2, group=group)
...
class GruopMorph(nn.Module):
    def conv_block(
        ...
        if batchnorm:
            ...
            layer = nn.Sequential(
                nn.Conv3d(
                    in_ch, 8, kernel_size, stride=stride, padding=padding, bias=bias
                ),
                nn.InstanceNorm3d(8),
                nn.LeakyReLU(0.2),
    ...
    def __init__(
        self, in_ch, basic_channel=8, imgshape=(160, 192, 192), groups=(4, 2, 2)
    ):
        ...
        self.groupflow8 = group_flow(
            27 + 64 // self.group8, basic_channel * 8, group=self.group8
        )
        self.groupflow4 = group_flow(
            27 + 32 // self.group4 + 32, basic_channel * 4, group=self.group4
        )
        self.groupflow2 = group_flow(
            27 + 16 // self.group2 + 16, basic_channel * 2, group=self.group2
        )
    ...
    def forward(self, x, y):
        ...
        costvolum8 = group_corr_volume(
            m8, f8, max_displacement=self.max_displacement1, group=self.group8
        )
        flow0, context0 = self.groupflow8(costvolum8, f8)  # context0 = 32 * 8
        # save_flow(flow0.cpu().numpy()[0, :, :, :, :].transpose(1, 2, 3, 0),
        #           'image_GDNet' + '/subfield' + str(8) + '_' + str(0) + '.nii.gz')
        smo = smo + smoothloss(flow0)
        ...
        for i in range(self.group8 // 2):
            contexti = self.context_attention8(
                contextps[2 * i],
                contextps[2 * i + 1],
                contexts[2 * i],
                contexts[2 * i + 1],
        ...
        context0 = upsample(context0, is_flow=False)  # [1,32,40,48,36]
        flow0 = upsample(flow0, is_flow=True)  # 3channel

        m4, f4 = self.split4(m4, f4)
        w4 = self.transformer(m4, flow0)
        costvolum4 = group_corr_volume(
            w4, f4, max_displacement=self.max_displacement1, group=self.group4
        )
        det_flow, context0 = self.groupflow4(costvolum4, f4, context0)
        smo = smo + smoothloss(det_flow)
        flow0 = self.transformer(flow0, det_flow) + det_flow
        # save_flow(flow0.cpu().numpy()[0, :, :, :, :].transpose(1, 2, 3, 0),
        #           'image_GDNet' + '/subfield' + str(4) + '_' + str(1) + '.nii.gz')
        ...
        for i in range(self.group4 // 2):
            contexti = self.context_attention4(
                contextps[2 * i],
                contextps[2 * i + 1],
                contexts[2 * i],
                contexts[2 * i + 1],
        ...
        flow0 = upsample(flow0, is_flow=True)  # 3channel
        context0 = upsample(context0, is_flow=False)  # [1,16,80,96,72]
        m2, f2 = self.split2(m2, f2)
        w2 = self.transformer(m2, flow0)
        costvolum2 = group_corr_volume(
            w2, f2, max_displacement=self.max_displacement1, group=self.group2
        )
        det_flow, context0 = self.groupflow2(costvolum2, f2, context0)
        # save_flow(det_flow.cpu().numpy()[0, :, :, :, :].transpose(1, 2, 3, 0),
        #           'image_GDNet' + '/subfield' + str(2) + '_' + str(0) + '.nii.gz')
        smo = smo + smoothloss(det_flow)
        flow0 = self.transformer(flow0, det_flow) + det_flow
        # save_flow(flow0.cpu().numpy()[0, :, :, :, :].transpose(1, 2, 3, 0),
        #           'image_GDNet' + '/subfield' + str(2) + '_' + str(1) + '.nii.gz')
        ...
        for i in range(self.group2 // 2):
            contexti = self.context_attention2(
                contextps[2 * i],
                contextps[2 * i + 1],
                contexts[2 * i],
                contexts[2 * i + 1],
        ...
        flow0 = upsample(flow0, is_flow=True)  # 3channel
        context0 = upsample(context0, is_flow=False)  # [1,8,160,192,144]
        warpedfeature2 = self.transformer(m1, flow0)
        x_in = torch.cat([warpedfeature2, f1, context0], dim=1)  # 27

        det_flow = self.conv(x_in)
        # save_flow(det_flow.cpu().numpy()[0, :, :, :, :].transpose(1, 2, 3, 0),
        #           'image_GDNet' + '/subfield' + str(1) + '_' + str(0) + '.nii.gz')
        smo = smo + smoothloss(det_flow)
        flow0 = self.transformer(flow0, det_flow) + det_flow
        # save_flow(flow0.cpu().numpy()[0, :, :, :, :].transpose(1, 2, 3, 0),
        #           'image_GDNet' + '/subfield' + str(1) + '_' + str(1) + '.nii.gz')
