...
from model.model import RCN_test
...
parser.add_argument(
    "--test_dir",
    type=str,
    default="/root/autodl-tmp/RDP_affineIXI_data/Test/",
    help="测试集数据目录",
)
parser.add_argument(
    "--altas_dir",
...
parser.add_argument("--num_labels", type=int, default=36, help="计算DSC的标签数")
...
def main():
    ...
    model_dir = f"result/{opt.model}_lr_{lr}/experiments/"

    img_size = opt.img_size

    model = RCN_test(img_size, flow_multiplier=2, n_cascade=n_cascade)
    ...
    test_set = datasets.IXIBrainInferDatasetS2Atlas(
        glob.glob(test_dir + "*.pkl"),
