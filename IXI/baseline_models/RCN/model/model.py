...
Original paper:
S. Zhao, Y. Dong, E. Chang, and Y. Xu, “Recursive cascaded networks
for unsupervised medical image registration,” in Proc. IEEE/CVF Int.
Conf. Comput. Vis. (ICCV), Oct. 2019, pp. 10599–10609.
"""
...
class UpConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=4, stride=2, final=False):
        ...
        if final:
            self.upconv.weight = nn.Parameter(
                Normal(0, 1e-5).sample(self.upconv.weight.shape)
            )
...
class VTN(nn.Module):
    ...
    def forward(self, moving, fixed):
        ...
        conv1, conv2, conv3, conv4 = self.encoder(concatImgs)
...
if __name__ == "__main__":
    model = VTN(inshape=(80, 96, 112)).cuda()
    # model = RCN(inshape=(160, 192, 160)).cuda()
    # print(str(model))
    A = torch.ones((1, 1, 80, 96, 112))
    B = torch.ones((1, 1, 80, 96, 112))
