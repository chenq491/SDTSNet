...
from model import function, losses
from model.model import RCN
...
parser.add_argument("--max_epoch", type=int, default=100, help="最大轮数")
parser.add_argument(
    "--cont_training", type=bool, default=False, help="是否从上次继续训练"
)
parser.add_argument("--learning_rate", type=float, default=0.0001, help="学习率")
parser.add_argument("--img_size", type=tuple, default=(80, 96, 112), help="数据尺寸")
parser.add_argument("--model", type=str, default="RCN", help="使用的模型名称")
parser.add_argument("--n_cascade", type=int, default=10, help="级联层数")
parser.add_argument(
    "--train_dir",
    type=str,
    default="/root/autodl-tmp/RDP_affineIXI_data/Train/",
    help="训练集数据目录",
)
parser.add_argument(
    "--val_dir",
    type=str,
    default="/root/autodl-tmp/RDP_affineIXI_data/Val/",
    help="验证集数据目录",
)
parser.add_argument(
    "--altas_dir",
    type=str,
    default="/root/autodl-tmp/RDP_affineIXI_data/Altas/",
    help="图谱数据目录",
)
...
def main():
    ...
    n_cascade = opt.n_cascade

    save_dir = "result/{}_lr_{}/".format(model_name, lr)
    ...
    Initialize model
    """
    model = RCN(img_size, flow_multiplier=2, n_cascade=n_cascade)
    model.cuda()
    ...
    train_set = datasets.IXIBrainDatasetS2Atlas(
        glob.glob(train_dir + "*.pkl"),
        glob.glob(altas_dir + "*.pkl"),
        transforms=train_composed,
    )
    val_set = datasets.IXIBrainInferDatasetS2Atlas(
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        Training
        """
        loss_all = function.AverageMeter()
        idx = 0
        ...
        for data in loop:
            ...
            loop.set_postfix(
                loss=loss.item(),
                ImgSim=loss_vals[0].item(),
                Reg=(loss.item() - loss_vals[0].item()),
            )
        ...
        """
        start_time = time.perf_counter()
        eval_dsc = function.AverageMeter()
        with torch.no_grad():
