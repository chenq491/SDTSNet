...
class UpConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=4, stride=2, alpha=0.1):
        ...
        self.actout = nn.Sequential(
            nn.InstanceNorm3d(out_channels), nn.LeakyReLU(alpha)
        )
...
class CConv(nn.Module):
    def __init__(self, channel):
        ...
        self.conv = nn.Sequential(ConvInsBlock(c, c, 3, 1), ConvInsBlock(c, c, 3, 1))
...
class RDP(nn.Module):
    def __init__(
        self, inshape=(160, 192, 160), flow_multiplier=1.0, in_channel=1, channels=16
    ):
        ...
        self.cconv_4 = nn.Sequential(
            ConvInsBlock(16 * c, 8 * c, 3, 1), ConvInsBlock(8 * c, 8 * c, 3, 1)
        )
        # warp scale 2
        self.defconv4 = nn.Conv3d(8 * c, 3, 3, 1, 1)
        self.defconv4.weight = nn.Parameter(
            Normal(0, 1e-5).sample(self.defconv4.weight.shape)
        )
        self.defconv4.bias = nn.Parameter(torch.zeros(self.defconv4.bias.shape))
        self.dconv4 = nn.Sequential(
            ConvInsBlock(3 * 8 * c, 8 * c), ConvInsBlock(8 * c, 8 * c)
        )
        ...
        self.defconv3.weight = nn.Parameter(
            Normal(0, 1e-5).sample(self.defconv3.weight.shape)
        )
        ...
        self.defconv2.weight = nn.Parameter(
            Normal(0, 1e-5).sample(self.defconv2.weight.shape)
        )
        ...
        self.defconv1.weight = nn.Parameter(
            Normal(0, 1e-5).sample(self.defconv1.weight.shape)
        )
    ...
    def forward(self, moving, fixed):
        ...
        flow = self.upsample_trilin(
            2 * (self.warp[1](flow, w) + w)
        )  # （1,3,160,196,160)
