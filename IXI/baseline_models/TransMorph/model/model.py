...
class TransMorph(nn.Module):
    ...
    def forward(self, source: torch.Tensor, target: torch.Tensor):
        ...
        source = x[:, 0:1, :, :]
        if self.if_convskip:
            x_s0 = x.clone()  # B C H W D
            x_s1 = self.avg_pool(x)  # B C H/2 W/2 D/2
            f4 = self.c1(x_s1)  # B C H/2 W/2 D/2
            f5 = self.c2(x_s0)  # B C H W D
        else:
            f4 = None
            f5 = None
        ...
        if self.if_transskip:
            f1 = out_feats[-2]  # B C H/8 W/8 D/8
            f2 = out_feats[-3]  # B C H/4 W/4 D/4
            f3 = out_feats[-4]  # B C H/2 W/2 D/2
        else:
            f1 = None
            f2 = None
            f3 = None
