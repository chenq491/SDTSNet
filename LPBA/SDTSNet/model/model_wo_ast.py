"""
SDTSNet WithOut All layer Semantic information Transfer
"""
...
class SDTSNet(nn.Module):
    def __init__(self, inshape=(160, 192, 160), in_channel=1, channels=8):
        ...
        self.upconv = nn.ModuleList(
            [
                nn.Sequential(UpConvBlock(2 * c, 4 * c), nn.Conv3d(4 * c, c, 3, 1, 1)),
                nn.Sequential(
                    UpConvBlock(4 * c, 8 * c), nn.Conv3d(8 * c, 2 * c, 3, 1, 1)
                ),
                nn.Sequential(
    ...
    def forward(self, moving: torch.Tensor, fixed: torch.Tensor):
        ...
        # decoder stage
        # layer 4
        flow, Ct = self.defconv[3](M4, F4)  # 16c 1/16  16c 1/16
        Ct = self.upconv[2](Ct)  # 4c 1/4

        # layer 3
        flow = self.upsample_trilin(2 * flow)  # 3 1/4
        warped = self.warp[2](M3, flow)
        residual_flow, Ct = self.defconv[2](warped, F3, Ct)  # 3 1/4  8c 1/4
        ...
        residual_flow, Ct = self.defconv[1](warped, F2, Ct)  # 3 1/2  4c 1/2
        ...
        residual_flow, _ = self.defconv[0](warped, F1, Ct)
