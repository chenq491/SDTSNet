"""
SDTSNet WithOut Stage Coase to Fine
"""
...
class Fusion(nn.Module):
    def __init__(self, inputc):
        ...
        self.conv1 = nn.Conv3d(inputc, 48, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv3d(48, 16, kernel_size=3, stride=1, padding=1)
        self.conv3 = nn.Conv3d(16, inputc, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, flows):
        ...
        flow = (
            flows.view(b, -1, 3, d, h, w) * nnf.softmax(k.view(b, -1, 3, d, h, w), 1)
        ).sum(1)
        return flow
...
class SDPM(nn.Module):
    ...
    def __init__(
        ...
        self.fusion = Fusion(3 * len(self.out_group_index))

    def forward(self, moving, fixed, Ct=None):
        h = None
        fmc = torch.cat([moving, fixed, self.corr(moving, fixed)], dim=1)
        flows = []
        for n, i in enumerate(self.out_group_index):
            x = self.convMain[0](fmc)
            ...
            flows.append(out_curr)

        flow = self.fusion(torch.cat(flows, dim=1))
        return flow, Ct
