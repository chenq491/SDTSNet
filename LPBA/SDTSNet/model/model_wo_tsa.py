"""
SDTSNet WithOut Temporal Semantic Aggregation
"""
...
class ConvLSTM(nn.Module):
    def __init__(self, inputc, hidden_dim):
        super().__init__()
        self.conv1 = nn.Conv3d(inputc, hidden_dim, 3, 1, 1, bias=True)
        self.conv2 = nn.Conv3d(hidden_dim, hidden_dim, 3, 1, 1, bias=True)
        self.conv3 = nn.Conv3d(hidden_dim, hidden_dim, 3, 1, 1, bias=True)
        self.conv4 = nn.Conv3d(hidden_dim, hidden_dim, 3, 1, 1, bias=True)
        self.lrelu = nn.LeakyReLU(0.1, inplace=True)

    def forward(self, h, x):
        hx = torch.cat([h, x], dim=1)
        h = self.lrelu(self.conv1(hx))
        h = self.lrelu(self.conv2(h))
        h = self.lrelu(self.conv3(h))
        h = self.lrelu(self.conv4(h))
        return h
...
class SDPM(nn.Module):
    ...
    def __init__(
        ...
        if use_corr:
            if last_layer:
                conv1 = nn.Sequential(
                    ConvInsBlock(c + 27, 3 * c),
                    ConvInsBlock(3 * c, c),
                    ConvInsBlock(c, c),
                )
            else:
                conv1 = nn.Sequential(
                    ConvInsBlock(c + c // 2 + 27, 3 * c),
                    ConvInsBlock(3 * c, c),
                    ConvInsBlock(c, c),
                )
        else:
            if last_layer:
                ...
                conv1 = nn.Sequential(
                    ConvInsBlock(c + c // 2, c), ConvInsBlock(c, c), ConvInsBlock(c, c)
                )
    ...
    def forward(self, moving, fixed, C=None):
        warped = moving
        out_prev = None
        H = None
        if C is not None:
            fm = torch.cat([moving, fixed, C], dim=1)
        else:
            fm = torch.cat([moving, fixed], dim=1)

        for n, i in enumerate(self.out_group_index):
            if self.use_corr:
                x = torch.cat([fm, self.corr(warped, fixed)], dim=1)
            else:
                x = fm
