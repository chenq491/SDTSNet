...
Original paper:
X. Hu, M. Kang, W. Huang, M. R. Scott, R. Wiest, and M. Reyes, “Dual
stream pyramid registration network,” in Proc. Int. Conf. Med. Image
Comput. Comput.-Assist. Intervent. Cham, Switzerland: Springer, 2019,
pp. 382–390.
"""
...
class BackBone(nn.Module):
    def __init__(self, in_channel=1, first_channel=8):
        ...
        self.decoder1 = DecoderBlock(
            deconv_channel=c * 4, skip_channel=c * 4, out_channel=c * 4
        )
        self.decoder2 = DecoderBlock(
            deconv_channel=c * 4, skip_channel=c * 2, out_channel=c * 4
        )
        self.decoder3 = DecoderBlock(
            deconv_channel=c * 4, skip_channel=c * 2, out_channel=c * 2
        )
        self.decoder4 = DecoderBlock(
            deconv_channel=c * 2, skip_channel=c, out_channel=c * 2
        )
...
class Correlation3D(nn.Module):
    ...
    def __init__(self, in_channel, kernel_size=3, d=3, sw=1, sf=2):
        ...
        self.sf = sf
        self.w = torch.ones(
            (in_channel, 1, self.kernel_size, self.kernel_size, self.kernel_size)
        ).cuda()
    ...
    def forward(self, mov, fix):
        ...
        pf = nnf.conv3d(
            fix, self.w, stride=self.sw, padding=self.sf + 1, groups=C
        )  # H+4
            ...
            for j in range(self.d):
                for k in range(self.d):
                    pf_crop = pf[
                        :,
                        :,
                        i * self.sf : (i * self.sf + H),
                        j * self.sf : (j * self.sf + W),
                        k * self.sf : (k * self.sf + T),
                    ]
...
class PRplusplusBlock(nn.Module):
    def __init__(
        self, size, in_channel, in_flow=True, scale=True, kernel_size=3, d=3, sw=1, sf=2
    ):
...
class PRNet(nn.Module):
    def __init__(self, size=(80, 96, 80), in_channel=1, first_channel=8):
        ...
        self.prblock2 = PRBlock(
            size=[s // 4 for s in size], in_channel=4 * c, in_flow=True, scale=True
        )
        self.prblock3 = PRBlock(
            size=[s // 2 for s in size], in_channel=2 * c, in_flow=True, scale=True
        )
...
class PRNetplusplus(nn.Module):
    def __init__(self, size=(80, 96, 80), in_channel=1, first_channel=8):
        ...
        c = first_channel
        self.net = BackBone(in_channel, c)
        self.prblock1 = PRplusplusBlock(
            size=None, in_channel=4 * c, in_flow=False, scale=False
        )
        self.prblock2 = PRplusplusBlock(
            size=[s // 4 for s in size], in_channel=4 * c, in_flow=True, scale=True
        )
        self.prblock3 = PRplusplusBlock(
            size=[s // 2 for s in size], in_channel=2 * c, in_flow=True, scale=True
        )
        self.prblock4 = PRplusplusBlock(
            size=size, in_channel=2 * c, in_flow=True, scale=True
        )
        self.prblock5 = PRplusplusBlock(
            size=size, in_channel=c, in_flow=True, scale=False
        )
